...
각 키에 해당하는 id 를 가져오고 가져온 id 에 해당하는 모든 속성을 FormInput 으로 전달

{...field} -> inputFields 에서 key 순서에 해당하는 id, label, placeholder 값을 FormInput 으로 전달

    onChange 이벤트 핸들러를 사용하지 않아도 되는가?!
    결론 => FormInput  내부에 onChange 를 추가해서 상태관리를 해야함
    이유 => onChange 가 없으면 사용자가 입력한 값을 저장할 수 없음
            onChange 의 역할 => 사용자가 입력한 값을 임시저장 -> 저장해놓은 값을 백앤드로 전달
...
const AddClothes = () => {
    ...
    예를 들어
    <input type="text" name="cName" value = {formData.cName} onChange={handleChange} 존재함
    ...
    */
    const handleChange = (e) => {
        const {name, value} = e.target;
        setFormData({
            ...formData, // ...이란? => 값이 변경된 다른 input 을 제외하고 값이 변경되지 않은 다른 input 값 상태를 그대로 유지한채로 값이 변경된 input 값만 변경
            [name]: value,
        })
    }
 ...
 */
    ...
    const inputFields = [
        {id: "cName", label : "의류명칭" , placeholder: "의류명칭을 입력하세요"},
        {id: "cCategory", label : "카테고리" , placeholder: "카테고리를 입력하세요"},
        ...
        {id: "cStock", label : "재고수량" , placeholder: "재고수량을 입력하세요"},
        {id: "cGender", label : "성별" , placeholder: "성별을 입력하세요"},
        {id: "cSeason", label : "시즌" , placeholder: "계절을 입력하세요"},
                    ...
                    <form id="contactForm">
                        ...
                        <div className="d-none" id="submitSuccessMessage">
                            <div className="text-center mb-3">
                                <div className="fw-bolder">등록 성공!</div>
                               등록한 제품 확인하기
                                <br/>
                        ...
                        <div className="d-none" id="submitErrorMessage">
                            <div className="text-center text-danger mb-3">의류를 등록하는데 문제가 발생하였습니다.!</div>
                        </div>

                        <div className="d-grid">
                            <button className="btn btn-primary btn-lg" id="submitButton" type="submit">등록하기
                            </button>
                        </div>
                    </form>
                </div>
